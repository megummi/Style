    ...
    def forward(self, content3_1, style3_1, content4_1, style4_1, content5_1, style5_1):
        ...
        return self.merge_conv(self.merge_conv_pad(
            self.sanet4_1(content4_1, style4_1) + self.upsample5_1(
                self.sanet5_1(content5_1, style5_1)) + self.pool(self.sanet3_1(content3_1, style3_1))))  # 把C，S的4_1，5_1经过注意力，将5_1的结果扩大两倍来匹配4_1，然后想加之后经过3*3的卷积
...
class Net(nn.Module):  # 本文的神经网络
    ...
    def forward(self, content, style):
        ...
        loss_c = self.calc_content_loss(g_t_feats[3], content_feats[3], norm=True) + self.calc_content_loss(
            g_t_feats[4], content_feats[4], norm=True) + self.calc_content_loss(g_t_feats[2], content_feats[2],
                                                                                norm=True)  # 计算内容损失
        ...
        for i in range(2, 5):
            if i == 2:
                ...
                mean = mean.view(b, h, w, -1).permute(0, 3, 1, 2).contiguous()
                std = std.view(b, h, w, -1).permute(0, 3, 1, 2).contiguous()
                Lf2 = self.mse_loss(mytargets, std * mean_variance_norm(content3) + mean)
                continue
            ...
            if i == 3:
                Lf3 = self.mse_loss(mytarget, std * mean_variance_norm(content_feats[i]) + mean)
            if i == 4:
                Lf4 = self.mse_loss(mytarget, std * mean_variance_norm(content_feats[i]) + mean) + Lf3 + Lf2
        ...
        for i in range(1, 5):
            l_identity2 += self.calc_content_loss(Fcc[i], content_feats[i]) + self.calc_content_loss(Fss[i],
                                                                                                     style_feats[i])
        return loss_c, loss_s, l_identity1, l_identity2, Lf4, loss_cx
...
parser.add_argument('--save_model_interval', type=int, default=10000)  # 每1000迭代存储一次
...
for i in tqdm(range(args.start_iter, args.max_iter)):  # tqdm实现进度条功能
    ...
    loss_c, loss_s, l_identity1, l_identity2, l_f, l_cx = network(content_images, style_images)  # 经过网络返回损失
    loss_c = args.content_weight * loss_c
    loss_s = args.style_weight * loss_s
    loss = loss_c + loss_s + l_identity1 * 50 + l_identity2 * 1 + 3 * l_f + l_cx * 3  # 总损失
