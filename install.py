...
class Transform(nn.Module):
    def __init__(self, in_planes):
        ...
        self.mattn=MAttention(in_planes)

    def forward(self, content4_1, style4_1, content5_1, style5_1):
        content4_1 = self.canet(content4_1)
        content5_1 = self.canet(content5_1)

        style4_1 = self.canet(style4_1)
        style5_1 = self.canet(style5_1)
...
parser.add_argument('--save_model_interval', type=int, default=5000)  # 每1000迭代存储一次
...
for i in tqdm(range(args.start_iter, args.max_iter)):  # tqdm实现进度条功能
    ...
    loss = loss_c + loss_s + l_identity1 * 50 + l_identity2 * 1 + 3 * l_f + l_cx * 3 + 500 * l_lap  # 总损失
