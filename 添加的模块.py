    ...
    def forward(self, content, style):
        ...
        for i in range(2, 5):
            if i == 2:
                ...
                saLf2 = (self.mse_loss(mytargets, sastyled3_1))/40
            ...
            if i == 3:
                Lf3 = self.mse_loss(mytarget, std * mean_variance_norm(content_feats[i]) + mean) + (self.mse_loss(mytarget, sastyled))/40
            if i == 4:
                Lf4 = self.mse_loss(mytarget, std * mean_variance_norm(content_feats[i]) + mean) + Lf3 + Lf2 + (self.mse_loss(mytarget, sastyled))/40
...
for i in tqdm(range(args.start_iter, args.max_iter)):  # tqdm实现进度条功能
    ...
    loss_c, loss_s, l_identity1, l_identity2, l_f,  l_cx,loss_lap = network(content_images, style_images)  # 经过网络返回损失
    loss_c = args.content_weight * loss_c
    loss_s = args.style_weight * loss_s
    loss = loss_c + loss_s + l_identity1 * 50 + l_identity2 * 1 + 3 *l_f   + l_cx * 3 + 5 * loss_lap # 总损失
    ...
    if i + 1 > 15000:
        ...
        if (i + 1) % args.save_model_interval == 0 or (i + 1) == args.max_iter or float(
            ...
            print((i + 1), float(loss.item()), float(loss_c.item()), float(loss_s.item()), float(l_id.item()),
                  float((0.1 * l_f).item()),float(3* l_cx.item()),float(5* loss_lap.item()), file=fp)
            fp.close()
